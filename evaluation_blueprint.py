...
def _synthesize_interactions(document: dict):
    ...
    try:
        ...
        with open(
            "prompt_templates/semantic_label_schema.json", "r", encoding="utf-8"
        ) as f:
            schema = json.load(f)
    ...
    cat_alias = {"シーン案": "シーン案", "キャラクター案": "キャラ案", "キャラ案": "キャラ案", "伏線案": "伏線案"}

    # ラベル名の揺らぎ吸収用マップ (default_interactions.json のラベル名 -> schema の日本語ラベル名)
    label_alias = {"内面葛藤": "内的葛藤深度", "成長度": "成長可能性", "伏線密度": "伏線強度", "回収確度": "回収明瞭性"}
    ...
    def add_to_map(inter_list, weight=1.0):
        for inter in inter_list:
            ...
            if key_a and key_b:
                ...
                interaction_map[pair] = interaction_map.get(pair, 0.0) + (
                    inter["strength"] * weight
                )
    ...
    for (ka, kb), strength in interaction_map.items():
        new_interactions.append(
            {"key_a": ka, "key_b": kb, "strength": round(strength, 1)}
        )
...
def auto_generate_interactions(doc_id):
    ...
    return jsonify(
        {
            "success": True,
            "interactions": document.get("evaluation_config", {}).get(
                "interactions", []
            ),
        }
    )
...
def apply_genre_presets():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": f"Failed to load presets: {str(e)}"}),
            500,
        )
    ...
    if not main_genre or main_genre not in presets:
        return (
            jsonify(
                {"success": False, "error": f"Valid main genre not set in document"}
            ),
            400,
        )
    ...
    # 有効な副ジャンルのプリセットのみを抽出
    valid_sub_presets = [
        presets[s] for s in sub_genres if s in presets and s != main_genre
    ]
