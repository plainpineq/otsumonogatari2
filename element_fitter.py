...
def _compute_single_fit(
...
) -> float:  # FeatureExtractor インスタンスを受け取る
    """
    ...
    for key in feature_extractor.vector_label_orders.keys():
        ...
        for i, score in enumerate(candidate_vector):
            if i < len(feature_extractor.vector_label_orders[key]) and score > 0:
                candidate_present_values.add(
                    feature_extractor.vector_label_orders[key][i]
                )
...
def apply_fit_to_candidates(
    candidates: List[Dict[str, Any]], ideal_elements: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
    """
    ...
    if (
        not feature_extractor.scalar_label_maps
        and not feature_extractor.vector_label_orders
    ):
    ...
    for candidate in candidates:
        ...
        for ideal in ideal_elements:
            ...
            score = _compute_single_fit(
                candidate_features, ideal_features, feature_extractor
            )
